...
use rustic_core::{
    BackendDynError, BackendResult, FileType, Id, ReadBackend, WriteBackend, ALL_FILE_TYPES,
};
...
impl FromStr for Throttle {
    ...
    fn from_str(s: &str) -> BackendResult<Self> {
        let mut values = s
            ...
            .map(|b| -> BackendResult<u32> { Ok(b?.as_u64().try_into()?) });
...
impl OpenDALBackend {
    ...
    pub fn new(path: impl AsRef<str>, options: HashMap<String, String>) -> BackendResult<Self> {
...
impl ReadBackend for OpenDALBackend {
    ...
    fn list_with_size(&self, tpe: FileType) -> BackendResult<Vec<(Id, u32)>> {
        ...
        Ok(self
            ...
            .into_iter()
            .filter(|e| e.metadata().is_file())
            .map(|e| -> BackendResult<(Id, u32)> {
                Ok((e.name().parse()?, e.metadata().content_length().try_into()?))
            })
            .filter_map(BackendResult::ok)
            .collect())
    }
...
impl WriteBackend for OpenDALBackend {
    ...
    /// * `id` - The id of the file.
    /// * `cacheable` - Whether the file is cacheable.
    fn remove(&self, tpe: FileType, id: &Id, _cacheable: bool) -> BackendResult<()> {
