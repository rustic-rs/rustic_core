...
fn test_backup_with_tar_gz_passes(
    ...
    // first backup  
    let first_snapshot = repo.backup(&opts, paths, SnapshotFile::default(), &cancel)?;
    ...
    let repo = repo.to_indexed_ids()?;
    // second backup  
    let second_snapshot = repo.backup(&opts, paths, SnapshotFile::default(), &cancel)?;
    ...
    let third_snapshot = repo.backup(&opts, paths, snap, &cancel)?;
...
fn test_backup_dry_run_with_tar_gz_passes(
    ...
    // dry-run backup  
    let snap_dry_run = repo.backup(&opts, paths, SnapshotFile::default(), &cancel)?;
...
/// The cancel token is set to `true` before the backup starts, so the backup
/// should return `ErrorKind::Cancelled` at the very first checkpoint and must
/// not write out any snapshot.
#[rstest]
fn test_backup_cancelled_writes_no_snapshot(
    tar_gz_testdata: Result<TestSource>,
    set_up_repo: Result<RepoOpen>,
) -> Result<()> {
    // Fixtures
    ...
    // Set the token to cancelled before the backup so the first checkpoint is
    // hit immediately, making the test deterministic (no thread race).
    let cancel = std::sync::Arc::new(std::sync::atomic::AtomicBool::new(true));

    // The backup should return an error instead of a snapshot.
    let err = repo
        .backup(&opts, paths, SnapshotFile::default(), &cancel)
        .unwrap_err();

    // Assert this is a cancellation error.
    // The guidance text is "The backup was cancelled by the user.", and the
    // Display output also contains the kind text "a cancelled operation";
    // both contain "cancel".
    assert!(
    ...
    // A cancelled backup must not persist any snapshot.
    let repo = repo.to_indexed_ids()?;
    let snapshots = repo.get_all_snapshots()?;
    assert!(
        snapshots.is_empty(),
        "cancelled backup must not persist a snapshot, found {}",
        snapshots.len()
    );
