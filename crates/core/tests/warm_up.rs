...
#[cfg(not(windows))]
#[rstest]
#[case(1, 10, 1900, 2300)] // batch_size=1, num_packs=10, sequential: ~2000ms
#[case(10, 10, 150, 500)] // batch_size=10, num_packs=10, parallel: ~200ms
...
fn test_warm_up_batch_default_value() {
    let options = RepositoryOptions::default();
    assert_eq!(
        options.warm_up_batch, DEFAULT_BATCH_SIZE,
        "warm_up_batch should default to {DEFAULT_BATCH_SIZE}"
    );
}
...
fn test_toml_deserialization_with_defaults() -> Result<()> {
 ...
 "#;
    ...
    assert_eq!(
        config.repo.warm_up_batch, DEFAULT_BATCH_SIZE,
        "Should default to {DEFAULT_BATCH_SIZE} when not specified"
    );
